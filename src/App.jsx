...
const App = () => {
  ...
  useEffect(() => {
    ...
    const fetchArticles = async () => {
      try {
        ...
        // You might want to handle the error more gracefully, e.g., show an error message
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleSubmit = (e) => {
    ...
    if (!text) {
      ...
      setQuery(text);
      // Commented out the following line to keep the search input value for reference
      // setText('');
    }
  };
      ...
      {isLoading ? (
        ...
        <>
          ...
          <p style={{ marginBottom: '1rem', color: 'white' }}>
            Category: <span style={{ fontWeight: 'bold', textTransform: 'capitalize' }}>{query}</span>
          </p>
